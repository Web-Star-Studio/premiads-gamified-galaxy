...
export const useMissionFilters = (missions: Mission[]) => {
  ...
  const findMissionsByBusinessType = (businessType: string) => missions.filter(mission => 
      mission.business_type?.toLowerCase().includes(businessType.toLowerCase())
    );

  // Find missions for specific gender
  const findMissionsByGender = (gender: string) => missions.filter(mission => 
      mission.target_audience_gender === gender || mission.target_audience_gender === 'all'
    );

  // Find missions by age range
  const findMissionsByAge = (age: number) => missions.filter(mission => 
  ...
  const findMissionsByRegion = (region: string) => missions.filter(mission => 
