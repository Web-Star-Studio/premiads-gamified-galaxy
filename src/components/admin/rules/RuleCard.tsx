...
const RuleCard = ({ rule, onToggle, onEdit, onSave, isEditing }: RuleCardProps) => {
  const getRuleValueInput = (rule: Rule) => {
    if (typeof rule.value === 'number') {
      return (
        <Input 
          type="number" 
          value={rule.value} 
          onChange={(e) => onSave(rule.id, parseInt(e.target.value))} 
      ...
      return (
        <Switch 
          checked={rule.value} 
          onCheckedChange={(checked) => onSave(rule.id, checked)} 
        />
      );
    } else {
      return (
        <Input 
          type="text" 
          value={rule.value} 
          onChange={(e) => onSave(rule.id, e.target.value)} 
