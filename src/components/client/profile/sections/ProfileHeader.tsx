...
export const ProfileHeader = ({ hasCompletedBefore, pointsAwarded }: ProfileHeaderProps) => (
    <div className="mb-6">
      ...
      <p className="text-gray-400">
        ...
        {!hasCompletedBefore && " Ganhe 10 tickets ao completar seu perfil pela primeira vez!"}
      </p>
      
      {pointsAwarded && (
        <div className="mt-4 p-3 bg-green-500/20 border border-green-500/30 rounded-md flex items-center gap-2">
          <CheckCircle className="text-green-500 h-5 w-5" />
          <span>Parabéns! Você ganhou {POINTS_PER_PROFILE_COMPLETION} tickets por completar seu perfil!</span>
        </div>
      )}
    </div>
  );
